...
export const getUnsubscribeHtml = ({
  company,
}: {
  user?: any;
  company?: any;
  ...
  return unsubscribeHtml.replace(/"/g, "'");
};
...
export const getUnsubscribeToolConfig = ({
  ...
  const unsubscribeHtml = getUnsubscribeHtml({ company });
  ...
  const renderer = `{
    renderer: {
      Viewer: window.unlayer.createViewer({
        render: (values) => values.html,
      }),
      exporters: {
        email: (values) => values.html,
      }
    }
  }`.replace(/\n/g, '');
